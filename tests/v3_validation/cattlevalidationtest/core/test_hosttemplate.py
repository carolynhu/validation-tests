...
import gdapi
import logging
# import machine  #machine package did not install
import time
...
@if_machine_digocean
def test_create_delete_do(hosttemplate, client):
    create_args = {
        "name": "test-do-ht1",
        "driver": "digitalocean",
        "secretValues": {
    ...
    ht1 = hosttemplate(create_args)
    client.wait_success(client.delete(ht1))


@if_machine_amazon
def test_create_delete_aws(hosttemplate, client):
    create_args = {
        "name": "test-aws-ht1",
        "driver": "amazonec2",
        "secretValues": {
...
@if_machine_amazon
@if_machine_digocean
def test_single_provider_only(hosttemplate, client):
    create_args = [{
        "name": random_str(),
        "secretValues": {
            "amazonec2Config": {
...
@if_machine_digocean
def test_create_host_nonexistent_hosttemplateid(host):
    host_name = random_str()
...
def test_create_host(client, hosttemplate, host):
    ...
    host0 = client.wait_success(host0.deactivate(), 60)
    client.wait_success(client.delete(host0), 60)
    client.wait_success(client.delete(ht0), 60)
...
def test_create_host_aws(client, hosttemplate, host):
    ...
    assert amazonec2_config["securityGroup"] == conf["Driver"]["SecurityGroup"]
